...
Since the current version of the code does not employ an iterative strategy for deformation, when you drag the shape, our deformation algorithm always starts from the initial positions. This can potentially result in artifacts if the operation is not performed carefully. Until I update the deformation algorithm, here are some editing tips to keep in mind. I kindly ask for your patience, as I will update the algorithm as soon as in my free time.
